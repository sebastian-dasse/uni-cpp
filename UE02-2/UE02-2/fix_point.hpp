...
template<int N> struct round_up_pow2 {
    static const int value = round_up_pow2<N+1>::value;
};
...
//    using DoubleType = typename int_type_for<L + R>::doubletype;
    ...
    using Type = typename int_type_for<round_up_pow2<L + R>::value>::type;
    using DoubleType = typename int_type_for<round_up_pow2<L + R>::value>::doubletype;

    Type m_data;
    fix_point(Type data); // QUESTION: any alternative to this private constructor?
};
